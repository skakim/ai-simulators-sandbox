		...
		for i in range(0, sub_episodes):
			...
			while running:
				...
				action = controller.take_action(
					AI.State(dist_enemy, int(enemy_in_fov), g_distance_to_bullet, int(g_bullet_in_fov)))
				...
				if running == False:
					if mode == "learn" and i == 19:
						controller.output(episode2, perf/20)
						controller.update(episode2, perf/20)
						episode2 += 1
					elif mode == "evaluate":
						episode += 1
