...
def versus(AI1, AI2):
	...
	state1 = P1.State(1,False,1,False)
	state2 = P2.State(1,False,1,False)
	player1, load1 = parse_info("./" + AI1 + "/group_info")
	player1 = player1.strip("\n")
	player2, load2 = parse_info("./" + AI2 + "/group_info")
	player2 = player2.strip("\n")
	controller1 = P1.Controller("./" + AI1 + "/" + load1, state1)
	controller2 = P2.Controller("./" + AI2 + "/" + load2, state2)
	...
	while(episode < 10):
		...
		while running:
			...
			action = controller1.take_action(controller1.state)
			actions += 1
			...
			controller2.state = P2.State(dist_enemy, int(gladiator_in_fov), e_distance_to_bullet, int(e_bullet_in_fov))	

			action = controller2.take_action(controller2.state)
